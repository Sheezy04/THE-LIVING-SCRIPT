...
| 来源 C | **汉典 zdic.net 字源演变**（甲骨文 3 格、楚系简帛 6 格） |
| 用途 | 01 字源探索、03 千年演变 |

当前 508 个已收录古文字槽位中，396 格来自本地 CDP 2.4，103 格来自
SIL OFL 1.1 的敬峰中山王篆，9 格来自汉典。汉典现行
[服务条款](https://zdic.net/terms/)声明站内字词等内容采用 CC0 1.0；
2026-09-25 已核对并更正此前“汉典全站无许可”的错误判断。
九格属于站点提供的字源演变描摹矢量，不是原拓；页面保留站方署名，
逐格出处及本地原文件哈希见 `assets/evidence/zdic/README.md`。
CC0 声明不替可能存在的上游第三方权利作保证，也不证明字形断代和释读正确。

**中研院素材的版本与使用依据（2026-09-25）**：本项目实际提取自官网列出的
[古漢字字型 2.4 版](https://cdp.sinica.edu.tw/download/dcatalog.htm)，没有取得或使用 2.65/2.7 完整安装包。
本地 `cdpfonts24.exe`（SHA-256 `B0E87A4E334642413045A4B02CA6059CD6FC71CF59FC585239DABD4324EA80F4`）
只含四个 TTF，没有随包许可文本；字体内部保留 `Proprietary for Sinica, all right reserved.` 字符串。
生成器只把下列代表字形轮廓写入 `data/ancient.js`，未在公开仓库分发原始 TTF：

| 2.4 字体 | 本项目取用／官网所列总字数 | 比例 |
| --- | ---: | ---: |
| `cdpjiagu.ttf` | 96／760 | 12.6% |
| `cdpbronz.ttf` | 99／1,533 | 6.5% |
| `cdpchubs.ttf` | 96／1,095 | 8.8% |
| `bnucdp.ttf`（北師大說文小篆） | 105／7,475 | 1.4% |

中研院的[公開釋出聲明](https://cdp.sinica.edu.tw/cdphanzi/declare.htm)标题为 2.65 版，
对该系统字体列出 GFDL 1.2、CC BY-SA 2.5 TW 使用方式，并列出单一类别字型取用不超过四分之一的合理使用参考范围。
参赛者确认按该公开取用范围作为本项目 2.4 字形节选的使用依据；这是参赛者的适用判断，
**不是**把 2.4 文件改称 2.65，也不表示中研院已逐件回信许可、为作品背书或已复核字形释读。
页面逐格标出 2.4 来源并链接声明原文；如有权利人进一步说明，以其正式说明复核并调整。
