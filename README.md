...
> **2026-09-25 成品口径：**首页与 01—06 六个模块；134 条教学目录中 106 字有可操作现代字形，28 字仅有义项及关系。五类古文字时间轴有 508/530 个代表字形格，22 格保持缺失；木、休、明另有独立例证与释形专题，专业复核仍待完成。作品支持离线浏览、挑战、自由创字与 PNG/SVG 导出。第三方素材及使用边界见 [资产清单](assets/vendor/LICENSES.md)。
>
> 下文保留早期开发记录，旧字数、测试次数、“未部署”等表述不代表当前状态。仓库只发布作品运行文件；参赛者内部的报名、团队与实测底稿未随公开仓库分发。
...
data/gap-notes.js       古文字 31 个缺格的查证记录（手工写的，只记事实、不含字形）
src/
  positioner.js         坐标系变换
...
下表**全部由 `node verify/run-all.mjs` 自动跑**（37 项 = 24 项 Node 层 + 8 项浏览器层
...
| 古文字五期 × 全字表归一化撑满 | `verify/test-ancient.html` | 499/499 字形撑满；最紧的一格 962.6（另有 9 格汉典按整张画布入库，走宽松判据）✓ |
...
```bash
node verify/run-all.mjs          # 全部 37 项，约 2 分钟（大头是开无头浏览器）
...
- **自检**：`verify/test-ancient.html` 断言古文字字形的**最长边恰好等于 962.6**
  （= 1024 × 0.94）。哪一格明显偏小，就是归一化漏了。当前 499/499 通过
  （另 9 格走下面的例外判据）
  （五期 × 106 字 = 530 格，缺 22 格 —— 史料所限，页面自己会说「本素材集未收录」，
  并在这行下面给出那一格查过哪些来源。依据在 `data/gap-notes.js`，与实测缺口**双向**钉住，
  见 `verify/test-gap-notes.mjs`：写了说明的格子必须真是缺口，缺口也必须都有说明。）
- **例外：汉典那 9 格不按墨迹归一化**（2026-09-22 补）。它们按汉典自己的 400×400
  整张画布入库，所以最长边是 870–907 而不是 962.6 —— 上面那条精确断言**跳过**它们，
  只查「没出框」与「没塌掉」，理由写在 `verify/zdic-outline.py` 文件头
  （汉典的署名落在墨迹之外，按墨迹归一化会把署名推出画布，或者把字形缩小近一半）。
  出处、授权状态与署名处置见 `data/gap-notes.js` 的文件头。
...
当前结果：**499 个古文字字形全部撑满、无出框、无过窄；另 9 格（汉典，按整张画布入库）
无出框、无塌陷，PASS。**
...
### 来源：汉典 zdic.net（CC0 1.0，已用 9 格）

<!-- 标题不编号：本文件里「来源之一」的「之一」后面并没有「之二」
     （中研院那一节叫「可接入：…」）。我加这一节时不去动那个旧编号，
     免得改了别人引用的标题 —— 但也不给自己编一个「之三」，那是假序号。 -->

**2026-09-25 复核更正**：汉典现行[服务条款](https://zdic.net/terms/)
明确声明站内字词等内容采用 CC0 1.0。先前“全站无授权声明”的结论有误，
已在数据源和界面改正。此声明仅记录汉典对本站内容的公开主张；不替可能存在的
上游第三方权利或逐字释读准确性作保证。

用它补的是原缺口 31 格中的 **9 格**：从/坐/鸡 @甲骨文，休/兔/李/鸡/好/男 @楚簡。
补完缺口 31 → 22。

有三件事必须一起说，不然「已经补上了」会被读成「没问题了」：

1. **那些图不是拓片，是描摹矢量。** 路径如 `/zy/jiaguwen/42_F5D6.svg`，
   `F5D6` 是 **PUA 私有区码位** —— 从哪个私有字体抽的，页面没写。字源段确实
   带真学术著录（集成 4225、郭.五.45 等），但著录与图是两回事。
2. **每张图都烧了汉典自己的署名**，而且是**矢量路径**（`grep '<text>'` 查不到）：
   4 张打 `ZIDIC.NET`，7 张打「漢典」。项目用单一 `--glyph-ink` 渲染，
   所以它们进画面后会变成深色、看得清清楚楚。**一个字形都没改，署名一个都没删**
   —— 这正是把它留下来而不是抹掉的意义。实测截图见
   `verify/tmp/zdic/marks.png`、`verify/tmp/zdic/app-cong.png`（后者是真页面）。
3. **归属如实登记**：`SOURCES.ZDIC` 标明 CC0 1.0 和许可原文链接，
   逐格对应在 `GLYPH_SRC`，运行时用 `AncientGlyphs.glyphSource(ch, era)` 取
   （⚠️ 用 `eraSource(era)` 会把那 3 个甲骨文格误署成中研院）。
   页面将汉典的 CC0 声明与 CDP 2.4 的使用依据分开登记，不混同来源。

**为什么那 9 格不按墨迹归一化**：汉典的署名落在墨迹之外（`从`：墨迹 x122–274，
署名 x385–396）。按墨迹归一化会把署名推出画布；把署名算进包围盒则字形要缩小近一半。
所以改用**汉典自己的 400×400 画布**当框，实测字形占 74%–93%，最长边 870–907
（正常是 962.6）。转换器是 `verify/zdic-outline.py`，文件头有完整推导。

**没补的两格是风格取舍，不是做不到**：狗@金文、猪@金文 汉典有，但给的是一整块
**黑底拓片**，与全站细线描边混在同一条演变带里会破相。想补的话把它们加进
`verify/build-ancient-data.py` 的 `ZDIC_SLOTS` 即可，SVG 已在 `verify/tmp/zdic/svg/`。
...
是 106 字（五期分别 99/99/102/103/105，其中 9 格来自汉典，见上一节），两者不是同一份数据，
别拿下表当 106 字的结论。
...
本地原件仍是官网列出的 **2.4 版**，字体内部保留
`Proprietary for Sinica, all right reserved.` 历史字符串；没有把它改称 2.65 版。
中研院的[公开释出声明](https://cdp.sinica.edu.tw/cdphanzi/declare.htm)标题为 2.65 版，
列出字体使用方式及单一类别取用不超过四分之一的合理使用参考范围。
本项目四类字形取用比例约 1.4%—12.6%，参赛者确认以该公开取用范围作为
2.4 字形节选的使用依据。界面逐格展示原始来源与依据链接；这不表示中研院
单独回信授权、为项目背书或完成文字学复核。文件哈希、逐类数字和边界见
...
| 甲骨文 / 金文 / 楚簡 / 小篆 | ✅ 已接入主时间轴（中研院 2.4 为主，另有汉典 9 格）；逐格展示来源与使用依据 |
| 隶书 | 主时间轴尚无可用矢量源；「出处与实证」另有 9 字的小学堂 CC0 汉代隶变查询图，不冒充主时间轴动画 |
...
5. Canvas 海报生成
6. 答辩材料附上 CDP 2.4 原件身份、逐类取用比例与中研院公开声明，且不宣称收到单独授权
